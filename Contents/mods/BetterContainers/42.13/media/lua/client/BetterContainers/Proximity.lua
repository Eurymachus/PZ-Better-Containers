...
local function liveLog(message)
    local msg = "[BetterContainers] " .. tostring(message)
    DebugLog.log(DebugType.General, tostring(msg))
end
...
function ISInventoryTransferAction:start(...)
    ...
    if playerNum ~= nil then
        ...
        -- Capture where this transfer was initiated from (Proximity vs forced container).
        --[[
        local lootWindow = getPlayerLoot and getPlayerLoot(playerNum)
        local page = lootWindow and lootWindow.inventoryPane and lootWindow.inventoryPane.inventoryPage or nil
        local invType = page and page.inventory and page.inventory:getType() or nil
        Proximity._LastTransfer[playerNum] = _isProximityType(invType) or false
        ]]
    end
...
function ISInventoryTransferAction:perform(...)
    ...
    if playerNum ~= nil and (self.started == false or not self.queueList or #self.queueList == 0) then
        ...
        local eff = Options.getEffectivePermissions() or {}
        if eff.proximityActive and eff.autoLock then
            if Proximity._LastTransfer[playerNum] then
                Proximity._LastBrowseMs[playerNum] = 0
            else
                Proximity._LastBrowseMs[playerNum] = getTimestampMs()
            end
        end

        Proximity._LastTransfer[playerNum] = nil
        ]]
    end
    ...
    for playerNum = 0, getNumActivePlayers() - 1 do
        ...
        if player then
            ...
            if last > 0 and remaining < Proximity._lastBrowseGraceMs then
                ...
                liveLog("[Better Containers] Don't snapback yet: " .. tostring(remaining) .. "ms remaining.")
            else
                local lootWindow = getPlayerLoot and getPlayerLoot(playerNum)
                local page = lootWindow and lootWindow.inventoryPane and lootWindow.inventoryPane.inventoryPage or nil
                if page then
                    liveLog(DebugType.General, "[Better Containers] Snap Back Now!")
                    Proximity.doAutoLock(playerNum, page, true)
                else
                    liveLog(DebugType.General, "[Better Containers] Failed Snap Back.")
                end
            end
        end
    end
