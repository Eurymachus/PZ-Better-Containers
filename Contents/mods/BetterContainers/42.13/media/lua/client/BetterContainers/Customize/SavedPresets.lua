...
local function _rowToPreset(sectionName, row)
    ...
    local icon = row.icon or row.item or row.texture
    ...
    if icon and icon ~= "" then payload.icon = tostring(icon) end
    if color then payload.color = color end

    -- If payload is empty, skip (prevents broken menu entries)
...
SavedPresets.saveNew = function(payload, meta)
    ...
    if payload.icon and tostring(payload.icon) ~= "" then
        row.icon = tostring(payload.icon)
    end
    ...
    INI.setOrdered(id, row, { "name", "icon", "r", "g", "b", "a" })
