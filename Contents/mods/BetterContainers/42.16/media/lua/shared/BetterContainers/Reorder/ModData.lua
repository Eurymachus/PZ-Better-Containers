...
ModData.getDefaultSortPriority = function(inventory, inventoryPage)
    ...
    local index = 0
    local increment = isFake and 1000 or 10

    for i, backpack in ipairs(inventoryPage.backpacks) do
        local inv = backpack.inventory
        if inv then
            local invIsFake = not inv:getContainingItem() and not inv:getParent()

            if invIsFake == isFake then
                index = index + 1
